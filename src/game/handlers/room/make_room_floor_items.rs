...
use crate::{
    encoding,
    game::room::{RoomItem, RoomModelStaticItem},
    message::outgoing::ROOM_STATIC_ITEMS,
};

pub async fn make_room_floor_items(room_floor_items: Vec<&RoomItem>) -> BytesMut {
    let mut bytes = BytesMut::with_capacity(8192);

    bytes.extend_from_slice(encoding::base64::encode(ROOM_STATIC_ITEMS, 2).as_slice());

    // Count
    bytes.extend_from_slice(encoding::wire::encode_i32(room_floor_items.len() as i32).as_slice());
